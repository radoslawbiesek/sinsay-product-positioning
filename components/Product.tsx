...
function calcDiscount(prices: [string, string]): number {
  console.log(prices)
  const values = prices.map(price => parseFloat(price.split(' ')[0].replace(',', '.')));
  values.sort();

  return (100 - Math.ceil(values[0] / values[1] * 100))
}

const Product = ({ product: { sku, imageUrl, title, prices } }: ProductProps) => {
  ...
  return (
    <Card style={{ width: '260px', textAlign: 'center', margin: '20px' }}>
      ...
      <Card.Title style={{ marginTop: 5 }}>
        <span>{title}</span>
        {discount > 0 && <Badge bg="danger" style={{ marginLeft: 10 }}>-{discount}%</Badge>}
      </Card.Title>
      <Card.Title>{sku}</Card.Title>
      <Card.Text>
        {isDiscount ?
          <>
            <span style={{ fontSize: '1.2rem' }} className="text-danger">{prices[0]} </span>
